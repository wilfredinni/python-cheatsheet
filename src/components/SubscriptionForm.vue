...
<template>
  <div v-if="showSubscription && !reader.isActive && showNewsletterForm"
    ...
    <p class="mb-1 text-center text-xl font-semibold text-slate-800 dark:text-slate-300 sm:text-start">
    ...
    <form class="mt-3 grid gap-y-3 sm:flex sm:space-x-3" @submit.prevent="subscribe">
      ...
      <button
        ...
        <svg v-else class="mx-auto h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
    ...
    <template v-if="response?.email">
      <p class="mx-3 mt-3 text-center text-sm font-medium text-sky-700 dark:text-sky-400">
    ...
    <template v-else-if="response?.success == false">
      <p class="mt-2 text-center text-sm font-medium text-sky-700 dark:text-sky-400">
